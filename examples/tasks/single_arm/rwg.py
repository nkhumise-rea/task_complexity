...
sys.path.insert(0, "..")
from task import SingleLink as one_link_arm
# from task_1 import SingleLink as one_link_arm
...
class NN(nn.Module): #actor_model
    def __init__(self,num_states,num_actions,num_hidden_l1,num_hidden_l2,act_limit):
        ...
        self.net =nn.Sequential(
            nn.Linear(num_states,num_hidden_l1,bias=True),
            nn.ReLU(),
            nn.Linear(num_hidden_l1,num_hidden_l2,bias=True),
            nn.ReLU(),
            nn.Linear(num_hidden_l1,num_actions,bias=True),
            nn.Tanh(),
            )
...
class RANDOM_AGENT():
    ...
    def evaluate(self):
        ...
        num_hidden_l1 = 256 #32 #5 
        num_hidden_l2 = 256 #32 #5 
...
if __name__ == '__main__':  
    ...
    np.save('{}_link_{}_dataset_{}_256Nom'.format(link_type,reward_type,args.count),trans_samples_df)
    ...
    plt.savefig('{}-link_{}_plots_{}_256Nom.png'.format(link_type,reward_type,args.count), dpi=300, bbox_inches='tight')
    # plt.show()
