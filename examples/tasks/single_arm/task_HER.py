...
class SingleLink(gym.Env):
    ...
    def angle_error(self,state):
        ...
        d = np.linalg.norm(state['achieved_goal'] - state['desired_goal'], 
                                 axis=-1)
        den = 2*l*r
        num = l**2 + r**2 - d**2
    ...
    def compute_state(self):
        ...
        end_effector_pos = np.array([s1,s2]) #[end_effector_coord_x,end_effector_coord_y]
        return {
            'observation':state, 
