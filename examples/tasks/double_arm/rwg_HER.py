...
from task import SingleLink as two_link_arm
"""
##locate task.py (modules)
import sys
sys.path.insert(0, "..")
from task_HER import DoubleLink as two_link_arm
...
class RANDOM_AGENT():
    def __init__(self,head=0):
        self.env = two_link_arm(head) #[0:GUI, 1:DIRECT]
    ...
    def evaluate(self):
        ...
        num_states = self.env.observation_space['observation'].shape[0]
        ...
        for episode in range(num_episodes):
            ...
            state = torch.tensor(obs['observation']).float() 
            # print('obs: ', obs)
            # print('state: ', state)
            # xxx
            ...
            #for _ in range(10):
                ...
                # print('state: ', state)
                #print(next_obs)
                next_state = torch.tensor(next_obs['observation']).float() 
                state = next_state
                #time.sleep(.01)
...
if __name__ == '__main__':  
    ...
    # np.save('{}_link_{}_dataset'.format(link_type,reward_type),trans_samples_df)
    ...
    # plt.savefig('{}-link_{}_plots.png'.format(link_type,reward_type), dpi=300, bbox_inches='tight')
    # plt.show()
