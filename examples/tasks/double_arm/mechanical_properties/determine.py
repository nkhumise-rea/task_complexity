...
for p in range(100000):
	joint_acc = (2*15)*random.random_sample((2,)) - 15 #[-15,15] (rad/s**2)
	joint_vel = (2*np.pi)*random.random_sample((2,)) - np.pi #[-pi,pi] (rad/s)
	joint_pos = (2*np.pi)*random.random_sample((2,))  #[0,2*pi] (rad)
	torques_est = FD(joint_acc,joint_vel,joint_pos).compute()
	link1_torques.append(torques_est[0])
