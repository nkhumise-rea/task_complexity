...
ALGORITHM_MAX = {
    ...
    '2linkd_obstacle': -43.09E-2, #link_arm < >
...
ALGORITHM_AVG = {
    ...
    '2linkd_obstacle': -67.902, #link_arm < >
...
def main():
    ...
    link_type = 2 #
    reward_type = 'dense_obstacle' #'dense' #'sparse'
    length = 170 #100 
    n_batch = 0 #0 1 2 3 4
    ...
    elif link_type == 2 and reward_type == 'dense_obstacle': env_name = '2linkd_obstacle'
    ...
    if link_type == 1: 
        parser.add_argument("--sourse_path", type=str, default='/{}link_{}_{}_batch/_2kBatch_{}.npy'.format(link_type,length,reward_type,n_batch))
    else:
        parser.add_argument("--sourse_path", type=str, default='/{}link_{}_batch/_2kBatch_{}.npy'.format(link_type,reward_type,n_batch))

    parser.add_argument("--root_dir", type=str, default='./batches')
    ...
    with open(os.path.join(output_dir, "{}_metrics_batch{}.txt".format(args.env,n_batch)), "w") as f:
        print("\t".join(_basic_columns), file=f)
    with open(os.path.join(output_dir, "{}_tables_batch{}.txt".format(args.env,n_batch)), "w") as f:
        print(" & ".join(_basic_columns), file=f)
    ...
    with open(os.path.join(output_dir, "{}_metrics_batch{}.txt".format(args.env,n_batch)), "a+") as f:
        print("\t".join(str(x) for x in values)+"\n", file=f)
    with open(os.path.join(output_dir, "{}_tables_batch{}.txt".format(args.env,n_batch)), "a+") as f:
