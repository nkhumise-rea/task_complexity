...
#modules
def roll_mean(array,window=40): #60
...
def group_plot(grids,means,stds):
    ...
    j = 100
    j2 = 40
    plt.plot(grids[0][:-j2],means[0][:-j2],color="deepskyblue",label='2link -SAC-[dense]')
    plt.fill_between(grids[0][:-j2],
                    means[0][:-j2] - stds[0][:-j2],
                    means[0][:-j2] + stds[0][:-j2],
                    facecolor="deepskyblue", 
                    alpha=0.1,
                    )  
    
    plt.plot(grids[1][:-j],means[1][:-j],color="purple",label='2link -SAC-[sparse]')
    plt.fill_between(grids[1][:-j],
                    means[1][:-j] - stds[1][:-j],
                    means[1][:-j] + stds[1][:-j],
                    facecolor="purple", 
                    alpha=0.1,
    ...
    plt.plot(grids[2][:-j],means[2][:-j],color="grey",label='2link -SAC+HER-[sparse]')
    ...
    plt.title('2-link sparse vs dense rewards',fontsize=f_size)
    ...
    # plt.rc('font',**{'size':'1'})
    # Access and modify the offset text (×10^3, ×10^4, etc.)
    ax = plt.gca()
    ax.xaxis.get_offset_text().set_fontsize(14)   # <-- change size here
...
batch_b1 = pd.read_csv('2link_d/2link_0.csv') #[:leng]
batch_b2 = pd.read_csv('2link_d/2link_1.csv')#[:leng]
batch_b3 = pd.read_csv('2link_d/2link_2.csv')#[:leng]
batch_b4 = pd.read_csv('2link_d/2link_3.csv')#[:leng]
batch_b5 = pd.read_csv('2link_d/2link_4.csv')#[:leng]

tl1 = 267 #273
# batch_c1 = pd.read_csv('2link_s/2link_SAC_0.csv')[:273]
...
batch_c5 = pd.read_csv('2link_s/2link_SAC_5.csv')[:tl1]

tl2 = 192 #185
# batch_d1 = pd.read_csv('2link_s/2link_SAC+HER0.csv')[:185]
...
batch_d5 = pd.read_csv('2link_s/2link_SAC+HER_5.csv')[:tl2]
...
# print('order_batch_b1: \n', order_batch_b1)

# gf = (order_batch_b1['Value']-order_batch_b1['Value'].min())/(order_batch_b1['Value'].max()-order_batch_b1['Value'].min())
# print('gf: \n', gf)
